...
{
    public class TheWorldOverHeavenStandT3 : StandClass
    {
        public override int PunchDamage => 298;
        public override int AltDamage => 611;
        ...
        private int realityOverwritePunchTarget = -1;
        private bool punchedWithRealityOverwrite = false;
        ...
        public override void AI()
        {
            ...
            if (mPlayer.standControlStyle == MyPlayer.StandControlStyle.Manual)
            {
                ...
                bool realityPunching = realityOverwritePunchTarget != -1;
                if (fPlayer.realityRewriteActive)
                {
                    if (realityOverwritePunchTarget == -1)
                    {
                        if (Main.mouseLeft && Projectile.owner == Main.myPlayer)
                        {
                            ...
                            for (int n = 0; n < Main.maxNPCs; n++)
                            {
                                ...
                                if (npc.active && npc.CanBeChasedBy(this, false))
                                {
                                    ...
                                    {
                                        Projectile.frame = 0;
                                        realityOverwritePunchTarget = npc.whoAmI;
                            ...
                            if (realityOverwritePunchTarget == -1 && shootCount <= 0)
                            {
                                shootCount += 60;
                                Main.NewText("Click on an enemy to overwrite them.", Color.White);
                            }
                        }
                    }
                    else
                    {
                        if (!Main.npc[realityOverwritePunchTarget].active || Main.npc[realityOverwritePunchTarget].life <= 0)
                        {
                            realityOverwritePunchTarget = -1;
                            return;
                        }

                        currentAnimationState = AnimationState.RealityOverwritePunch;
                        if (!punchedWithRealityOverwrite && Vector2.Distance(Main.npc[realityOverwritePunchTarget].Center, Projectile.Center) >= Main.npc[realityOverwritePunchTarget].width)
                        {
                            Vector2 velocityAddition = Main.npc[realityOverwritePunchTarget].Center - Projectile.Center;
                            velocityAddition.Normalize();
                            velocityAddition *= 6f + mPlayer.standTier;
                            Projectile.velocity = velocityAddition;
                        }
                        else
                            Projectile.velocity = Vector2.Zero;

                        if (!punchedWithRealityOverwrite && Projectile.owner == Main.myPlayer)
                        {
                            if (Vector2.Distance(Main.npc[realityOverwritePunchTarget].Center, Projectile.Center) < Main.npc[realityOverwritePunchTarget].width)
                            {
                                if (Projectile.frame == 3)
                                {
                                    punchedWithRealityOverwrite = true;
                                    NPC.HitInfo realityPunchHitInfo = new NPC.HitInfo()
                                    {
                                        Damage = newPunchDamage * 3,
                                        Knockback = 8f,
                                        HitDirection = Projectile.direction
                                    };
                                    Main.npc[realityOverwritePunchTarget].StrikeNPC(realityPunchHitInfo);
                                    NetMessage.SendStrikeNPC(Main.npc[realityOverwritePunchTarget], realityPunchHitInfo, Projectile.owner);
                                    Main.npc[realityOverwritePunchTarget].AddBuff(ModContent.BuffType<RealityRewriteBuff>(), 30 * 60);
                                    ...
                                    for (int i = 0; i < amountOfDusts; i++)
                                    {
                                        ...
                                        int dustIndex = Dust.NewDust(Main.npc[realityOverwritePunchTarget].Center, 1, 1, DustID.WhiteTorch, Scale: 1.6f);
                ...
                if (SecondSpecialKeyPressed())
                {
                    ...
                    if (fPlayer.realityRewriteActive)
                    {
                        player.AddBuff(ModContent.BuffType<RealityRewriteBuff>(), 2);
        ...
        public override void SendExtraStates(BinaryWriter writer)       //since this is overriden you have to sync the normal stuff
        {
            ...
            writer.Write(Main.player[Projectile.owner].GetModPlayer<FanPlayer>().realityRewriteActive);
            writer.Write((short)realityOverwritePunchTarget);
        }
        ...
        public override void ReceiveExtraStates(BinaryReader reader)
        {
            ...
            Main.player[Projectile.owner].GetModPlayer<FanPlayer>().realityRewriteActive = reader.ReadBoolean();
            realityOverwritePunchTarget = reader.ReadInt16();
        }
        ...
        public override void SelectAnimation()
        {
            ...
            if (currentAnimationState == AnimationState.Idle)
                ...
                PlayAnimation("Snap");
            else if (currentAnimationState == AnimationState.RealityOverwritePunch)
                PlayAnimation("RealityOverwrite");
        ...
        public override void AnimationCompleted(string animationName)
        {
            ...
            if (currentAnimationState == AnimationState.RealityOverwritePunch)
            {
                punchedWithRealityOverwrite = false;
                Main.player[Projectile.owner].GetModPlayer<FanPlayer>().realityRewriteActive = false;
                Main.player[Projectile.owner].AddBuff(ModContent.BuffType<AbilityCooldown>(), Main.player[Projectile.owner].GetModPlayer<MyPlayer>().AbilityCooldownTime(8));
                realityOverwritePunchTarget = -1;
            }
        }
